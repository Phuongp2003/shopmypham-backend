...
import { LoginDto, RegisterDto, RefreshTokenDto, AuthResponseDto } from '@/modules/auth/auth.dto';
...
export class AuthService {
	static async login(input: z.infer<typeof LoginDto>): Promise<AuthResponseDto> {
		try {
			...
			// Generate tokens
			const tokenPayload: AuthPayload = { id: user.id, email: user.email, role: user.role, name: user.name, googleId: user.googleId || undefined };
			const tokenOptions: SignOptions = { expiresIn: AuthMiddleware.ACCESS_TOKEN_EXPIRES_IN };
	...
	static async register(input: z.infer<typeof RegisterDto>): Promise<AuthResponseDto> {
		try {
			// Validate input
	...
	static async refreshToken(input: z.infer<typeof RefreshTokenDto>): Promise<AuthResponseDto> {
		try {
			const { refreshToken } = RefreshTokenDto.parse(input);

			const decoded = jwt.verify(refreshToken, AuthMiddleware.REFRESH_TOKEN_SECRET) as AuthPayload;
