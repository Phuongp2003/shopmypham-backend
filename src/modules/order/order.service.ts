...
export class OrderService {
    ...
    async getOrders(query: OrderQueryDto) {
        const { status, userId, page = 1, limit = 10, sortBy = 'createdAt', sortOrder = 'desc' } = query;
    ...
    async cancelOrder(id: string) {
        ...
        if (order.status !== OrderStatus.PENDING) {
            throw new HttpException(HttpStatus.BAD_REQUEST, 'Only pending orders can be cancelled');
        }
