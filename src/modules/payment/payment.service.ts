...
import { CreatePaymentDto, MOMOPaymentRequest, MOMOPaymentResponse, MOMOPaymentCallback } from './types/payment.types';
...
export class PaymentService {
    ...
    async createMOMOPayment(data: MOMOPaymentRequest): Promise<MOMOPaymentResponse> {
        const accessKey = process.env.MOMO_ACCESS_KEY;
        const secretKey = process.env.MOMO_SECRET_KEY;
        const partnerCode = process.env.MOMO_PARTNER_CODE;

        if (!accessKey || !secretKey || !partnerCode) {
            throw new HttpException(HttpStatus.INTERNAL_SERVER_ERROR, 'MOMO configuration missing');
        }
        ...
        try {
            const response = await axios.post(process.env.MOMO_URL_CREATE!, requestBody);
            return response.data;
        } catch (error) {
            throw new HttpException(HttpStatus.INTERNAL_SERVER_ERROR, 'Failed to create MOMO payment');
        }
    }

    async handleMOMOCallback(data: MOMOPaymentCallback) {
