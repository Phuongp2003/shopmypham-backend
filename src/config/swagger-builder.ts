...
export class SwaggerBuilder {
	...
	addSchemasFromComments(files: string[]): SwaggerBuilder {
		...
		for (const file of files) {
			...
			const regex = /\/\*\*([\s\S]*?@swagger[\s\S]*?)\*\/\s*(export (type|interface|class) (\w+))/g;
			let match;
			while ((match = regex.exec(content)) !== null) {
				const comment = match[1];
				const typeName = match[4];
				// Lấy phần sau @swagger làm yaml
				const yamlText = comment.split('@swagger')[1].replace(/^\s*\* ?/gm, '');
				try {
	...
	addControllersFromAnnotations(controllers: Function[]): SwaggerBuilder {
		...
		for (const ctrl of controllers) {
			...
			const instanceMethodNames = Object.getOwnPropertyNames(proto).filter(
			...
			if (schema && schema.properties) {
				for (const [prop, propSchemaRaw] of Object.entries(schema.properties)) {
					...
					const param: any = {
						...
						name: prop,
						schema: { ...propSchema },
						required: Array.isArray(schema.required) ? schema.required.includes(prop) : false,
					};
					if (propSchema.enum) {
				...
				parameters.push({
					...
					schema: { $ref: `#/components/schemas/${methodMeta.query}` },
				});
			}
		}
		// Path params
