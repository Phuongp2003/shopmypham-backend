...
export class AuthMiddleware {
  static readonly ACCESS_TOKEN_SECRET: jwt.Secret = process.env.JWT_SECRET || 'your-secret-key';
  static readonly REFRESH_TOKEN_SECRET: jwt.Secret = process.env.JWT_REFRESH_SECRET || 'your-refresh-secret-key';
  ...
  static async authenticate(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      try {
        // Verify access token
        const decoded = jwt.verify(accessToken, this.ACCESS_TOKEN_SECRET) as jwt.JwtPayload;
        req.user = {
          id: decoded.id,
          email: decoded.email,
          name: decoded.name,
        ...
        const decoded = jwt.verify(refreshToken, this.REFRESH_TOKEN_SECRET) as jwt.JwtPayload;
  ...
  static async logout(req: Request, res: Response) {
    try {
      ...
      if (redis && refreshToken) {
        const decoded = jwt.verify(refreshToken, this.REFRESH_TOKEN_SECRET) as jwt.JwtPayload;
        const expiresIn = decoded.exp ? decoded.exp - Math.floor(Date.now() / 1000) : 7 * 24 * 60 * 60; // 7 days
